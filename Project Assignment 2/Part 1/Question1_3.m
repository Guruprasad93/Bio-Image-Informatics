...
function[imGauss] = Question1_3(pathToImage,sigma_x, sigma_t, phi)
...
for phiVal = phi
    ...
    imPad = padarray(im,[floor(Nt/2) floor(N/2)]);
    ...
    figure(1)
    imshow(imGauss_x(floor(N/2)+1: size(imPad,1)-floor(N/2),floor(N/2)+1:size(imPad,2)-floor(N/2)),[]);
    title(strcat('Blurred Image along x dir for sigma = ',' ',num2str(sigma_x)));
...
%      end    
    ...
    %Along T direction
    for x = floor(N/2)+1:size(imPad,2)-floor(N/2)
        for y = floor(N/2)+1: size(imPad,1)-floor(N/2)
    ...
    title(strcat('Blurred Image along x+t dir for sigma = ',' ',num2str(sigma_x)));
